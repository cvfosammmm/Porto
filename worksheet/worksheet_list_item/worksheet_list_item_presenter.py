...
class WorksheetListItemPresenter(object):

    def __init__(self, worksheet, item, sb_view, hb_view):
    ...
    def change_notification(self, change_code, notifying_object, parameter):
        ...
        if change_code == 'save_state_change':
            self.update_title()
            self.sb_view.set_last_saved(self.worksheet.get_last_saved())
            self.hb_view.set_last_saved(self.worksheet.get_last_saved())
    ...
    def update_title(self):
        save_state = '*' if self.worksheet.get_save_state() == 'modified' else ''
        self.sb_view.set_name(save_state + self.worksheet.get_name())
        self.hb_view.set_name(save_state + self.worksheet.get_name())

    def update_subtitle(self):
        busy_cell_count = self.worksheet.get_busy_cell_count()
        if busy_cell_count > 0:
            plural = 's' if busy_cell_count > 1 else ''
            subtitle = 'evaluating ' + str(busy_cell_count) + ' cell' + plural + '.'
        elif self.worksheet.get_kernel_state() == 'starting':
